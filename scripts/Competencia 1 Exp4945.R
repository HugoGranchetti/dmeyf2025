...
vars_excluir <- c("numero_de_cliente", "foto_mes", "clase_ternaria")
vars_num <- names(dataset)[sapply(dataset, is.numeric)]
vars_num <- setdiff(vars_num, vars_excluir)

cat("Variables a rankear:", length(vars_num), "\n")

# Solo ranking por mes
for (v in vars_num) {
  rank_col <- paste0(v, "_rank")
  dataset[, (rank_col) := frank(get(v), na.last = "keep", ties.method = "random"), 
          by = foto_mes]
}

cat("✅ Rankings creados:", length(vars_num), "features\n")
